package com.doodle.core.advertising.components
...
import com.doodle.core.advertising.rewarded.rememberRewardedAdViewState
...
fun WatchAdForWallpaper(
    ...
    val rewardedInterstitialAdState = rememberRewardedAdViewState(
    ...
    LaunchedEffect(showAdToUser.isVisible) {
        if (showAdToUser.isVisible) {
            rewardedInterstitialAdState.showAd { result ->
                showAdToUser.dismiss()
                when (result) {
                    AdShowResult.SHOWN_OR_REWARD_EARNED -> onWatched(args)
                    AdShowResult.ERROR -> {
                        scope.launch {
